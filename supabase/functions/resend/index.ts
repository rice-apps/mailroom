...
const handler = async (request: Request): Promise<Response> => {
  ...
  const callerResp = await supabase.auth.getUser(token);
  if (!callerResp.data.user) {
    return new Response("Unauthorized", { status: 401, headers: corsHeaders });
  }
  ...
  const callerUserResp = await supabase
    ...
    .eq("email", callerResp.data.user.email)
    .single();
  ...
  if (userResp.error) {
    ...
    return new Response(`Error fetching user: ${userResp.error}`, {
      status: 500,
    });
  }

  const emails = [netID + "@rice.edu"];
  if (userResp.data.additional_email) {
    emails.push(userResp.data.additional_email);
  }
