...
export default function NavigationBar() {
  ...
  const showBackButton = !pathname.includes('/kiosk') && !pathname.includes('/admin') && !pathname.includes('/home');
  ...
  useEffect(() => {
    const checkForOverlaps = () => {
      ...
      const elementsNearTop = Array.from(document.querySelectorAll('h1, h2, .card, [role="heading"]'))
        .filter(el => {
